...
- [ ] pnpm workspace monorepo: `apps/web`, `apps/api`
- [ ] `apps/api`: NestJS init, `/health` endpoint returning `{ ok: true }`
- [ ] `apps/web`: Next.js App Router + Tailwind + shadcn/ui init
- [ ] Supabase project created; Postgres connection strings noted
      (pooled `:6543` for runtime, direct `:5432` for migrations)
- [ ] Supabase Storage: private bucket `data-room-files` created
- [ ] `.env.example` in both apps listing every variable, no secrets
...
- [ ] CORS on the API: explicit origin allowlist, `credentials: true`
